...
6. **Memory** — memory-aware observability, memory health, memory analytics, and memory report export.
7. **Upload** — JSON, CSV, ZIP, and bulk telemetry ingestion with preview and statistics.
8. **Run timeline** — task-level run timeline using start timestamp and latency.
9. **Raw data** — filtered telemetry table for inspection and export workflows.
